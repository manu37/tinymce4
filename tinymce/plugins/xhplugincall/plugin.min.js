(function () {
    ...
    editor.on('BeforeSetContent',function(e){
      if (e.load) {
        e.content=e.content.replace(/\{{3}(.*?)\}{3}/g,function(match){
//          alert('BeforeSetContent: ' + match)
          return '<span class="xhplugincall mceNonEditable">' + match + '</span>';
        });
      }
    });
    editor.on('GetContent',function(e){
      if (e.save) {
        e.content = e.content.replace(/\<span class="xhplugincall mceNonEditable"\>+?(.*?)\<\/span\>+?/g,function(match,sub1){
//          alert('GetContent: ' + sub1)
          return sub1;
        });
      }
    });
